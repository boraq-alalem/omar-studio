...
export function ReservedTitlesTable({ titles, onEdit, onDelete, className }: ReservedTitlesTableProps) {
    ...
    return (
      <div className={cn("space-y-3 px-2", className)}>
        {titles.map((title, index) => (
          <div key={title.id} className="bg-card rounded-xl border border-border/50 p-4 shadow-sm" style={{ animationDelay: `${index * 0.05}s` }}>
            <div className="space-y-3">
              <div className="space-y-1">
                <h4 className="font-semibold text-sm leading-tight line-clamp-2 text-right">
                  {title.title}
                </h4>
                <div className="flex justify-end">
                  <div className="h-1 w-8 bg-gradient-to-r from-blue-500/30 to-purple-500/30 rounded-full"></div>
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="space-y-1">
                  <span className="text-muted-foreground font-medium">الطالب:</span>
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 bg-gradient-to-br from-green-500 to-teal-600 rounded-lg flex items-center justify-center text-white font-semibold text-xs">
                      {title.student_name.charAt(0).toUpperCase()}
                    </div>
                    <span className="font-medium text-xs truncate">{title.student_name}</span>
                  </div>
                </div>
                
                <div className="space-y-1">
                  <span className="text-muted-foreground font-medium">المشرف:</span>
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center text-white font-semibold text-xs">
                      {title.supervisor_name.charAt(0).toUpperCase()}
                    </div>
                    <span className="font-medium text-xs truncate">{title.supervisor_name}</span>
                  </div>
                </div>
                
                <div className="space-y-1">
                  <span className="text-muted-foreground font-medium">الدرجة:</span>
                  <div className="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-gradient-to-r from-purple-500/10 to-pink-500/10 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-700">
                    {title.degree}
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-xs font-medium">{formatDate(title.reservation_date)}</span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <p className="text-xs font-medium truncate" title={title.specialization}>{title.specialization}</p>
                </div>
                <div>
                  <span className="text-muted-foreground font-medium text-xs">الجامعة:</span>
                  <p className="text-xs font-medium truncate" title={title.university}>{title.university}</p>
                </div>
              </div>
                ...
                <div className="flex gap-2 pt-2 border-t border-border/30">
                  {onEdit && (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-yellow-600 bg-yellow-50 hover:bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-400 rounded-lg transition-colors"
                    >
                  ...
                  {onDelete && (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 rounded-lg transition-colors"
                    >
  ...
  const getStatusColor = (status?: string) => {
    switch (status?.toLowerCase()) {
      ...
      case 'expired':
      case 'منتهي':
        return 'from-red-500/10 to-rose-500/10 text-red-700 dark:text-red-300 border-red-200 dark:border-red-700';
      case 'pending':
      case 'معلق':
        return 'from-yellow-500/10 to-amber-500/10 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-700';
      default:
        return 'from-gray-500/10 to-gray-600/10 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-700';
    }
  };
